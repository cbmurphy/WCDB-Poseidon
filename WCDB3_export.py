...
if __name__ == "__main__":
	...
	if len(sys.argv) > 1:
		arg = sys.argv[1]
		try:
			if isinstance(arg, str) and arg[-4:] == ".xml":
				print "Using new file %s" %arg
				file = sys.argv[1]
			if isinstance(arg, bool)and arg != 0:
				print "Dropping all tables"
			else:
				print "Merging to existing data"
		except:
			pass
	c = login()
	tree = ET.parse(file)
	...
	# import
	wcdb3_import(root,drop_all,use_current)
