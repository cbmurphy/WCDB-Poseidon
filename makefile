...
diff:
	RunWCDB3.py < WCDB3_input_list.txt > RunWCDB3.tmp
...
run:
	RunWCDB3.py < WCDB3_input_list.txt > RunWCDB3.out.xml

test:
	TestWCDB3.py
	
validate:
	xmllint --noout --schema WCDB2.xsd.xml xmls/*.xml
	
query:
	mysql -h z -u xiaolong -pmTnYneXHjw -H < WCDB3q.sql > WCDB3q.html
...
zip:
	...
	WCDB3d.html WCDB3q.html \
	WCDB3.log WCDB3.py WCDB3u.pdf WCDB3q.sql
